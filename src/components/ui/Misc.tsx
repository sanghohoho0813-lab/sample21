...
export function Term({ children, term }: { children?: ReactNode; term: string }) {
  ...
  return (
    <span className="relative inline-flex items-center gap-0.5">
      {children ?? term}
      {desc && (
        <button type="button" aria-label={`${term} 설명`} onClick={() => setOpen((v) => !v)} onBlur={() => setOpen(false)} className="text-neutral-text2 hover:text-theme-primary inline-flex items-center justify-center h-10 w-7 -my-3 md:h-5 md:w-5 md:my-0"><HelpCircle size={14} /></button>
      )}
